...
{
    if (!m_fileName.isEmpty()) {
        QDesktopServices::openUrl(QUrl::fromLocalFile(DOWNLOAD_DIR.filePath(m_fileName)));
...
void Downloader::installUpdate()
{
    ...
    box.setText("<h3>" + tr("In order to install the update, you may need to "
...
void Downloader::saveFile(qint64 received, qint64 total)
{
    ...
    if (received >= total && total > 0) {
        /* Rename file */
        QFile::rename(DOWNLOAD_DIR.filePath(m_fileName + PARTIAL_DOWN), DOWNLOAD_DIR.filePath(m_fileName));
